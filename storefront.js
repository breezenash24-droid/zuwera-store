...
(function() {
  ...
  if (isBuilderPreview) {
    ...
    /* The on-canvas inline TEXT editor used to live here, and therefore only
       on the homepage -- storefront.js is index.html's script. The builder
       previews the product, collection and landing pages too, and the nav
       and announcement bar appear on ALL of them, so an editor that only
       existed on one page could never reach the chrome. It moved to
       zw-copy.js, which every storefront page loads. */
