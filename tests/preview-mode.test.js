...
(async function () {
  ...
  console.log('\n  what a token can reach');
  {
    ...
    ok('only draft storefront keys are readable', list.length > 0 &&
      list.every(k => /^(page_builder|landing_pages|builder_theme|builder_nav|product_page_draft|collection_page_draft|nav_menu_draft|announcement_bar_draft|text_overrides_draft|header_layout_draft|bag_panel_draft)$/.test(k)),
      list.join(','));
