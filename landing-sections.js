...
(function () {
  ...
  window.zwEnsureSwipeArrows = window.zwEnsureSwipeArrows || function (wrap, grid) {
    ...
    // title (far right), so they sit in open space and never crowd the cards. Fall
    // back to a band above the row only when the section has no .sec-head header.
    ...
    if (head) {
      ...
      wrap.classList.add('zw-arrows-in-head');
    } else {
